...
<h4 style="text-align:center;width:100%;border-top:1px solid;border-bottom:1px solid;" class="card-header" >My Courses Collection</h4>
...
<div class="row justify-content-center" style="margin-top:10px;">
	<?php
																			...
																			$query= "SELECT * FROM `orderitem` where username='$username'";
																			$count=0;
																			$result4=mysqli_query($conn,$query);
																						while($row=mysqli_fetch_assoc($result4))
																						{
																							$coursename= $row['course_name'];
																							$qury= "SELECT * FROM `items` where id='$coursename'";
																							$reslt=mysqli_query($conn,$qury);
																						...
																						//	if($row2['id']==$row['category'])
																							...
																							<div class="card-body">
																										 <img class="d-block w-100" height="200px;" src="Admin/'.$image.'" alt="">
																							  <div class="well">
																							  <h5>'.$name.'</h5>
																								<p style="width:100%;"> <span style="float:right">Rating '.$stock.'</span></p>
																								<br>
																									</div>	
																							<form action="viewcourse.php" method="POST" style="display:block;">
																							<input type="text" value="'.$id.'" style="display:none;" name="id">
																							<input type="submit" class="btn"   value="View" style="width:100%;text-align:center; color:white; background-color:#3E065F;" >
																							</form>	
